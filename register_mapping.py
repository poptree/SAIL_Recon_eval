...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base_seed", type=int, default=1305, help="seed to control randomness"
    )
    ...
    parser.add_argument(
        "--max_estimates", type=int, default=-1, help="max number of images to consider"
    )
    ...
    if opt.render_visualization:
        ...
        ace_visualizer.setup_reloc_visualisation(
            frame_count=len(testset), camera_z_offset=opt.render_camera_z_offset
        )
    else:
        ace_visualizer = None
    ...
    for estimate in ace_estimates:
        ...
        dataset_io.write_pose_to_pose_file(
            ...
            pose=out_pose,
            confidence=estimate.confidence,
            focal_length=estimate.focal_length,
        )
    ...
    if ace_visualizer is not None:
        ...
        state_in_file = os.path.join(
            opt.render_target_path, opt.network.stem + "_mapping.pkl"
        )
        state_out_file = os.path.join(
            opt.render_target_path, opt.network.stem + "_register.pkl"
        )
