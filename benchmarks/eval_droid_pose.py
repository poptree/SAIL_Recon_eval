...
if __name__ == "__main__":
    ...
    for scene_folder in scene_folders:
        ...
        aligned_poses = scene_folder / 'poses_filter.txt'
        given_path = scene_folder/ "poses.txt"

        c2ws_opencv = np.loadtxt(given_path).reshape(-1,3,4)
        bottom = np.array([[0, 0, 0, 1]]).reshape(1,1,4)
        c2ws_opencv = np.concatenate([c2ws_opencv, bottom.repeat(c2ws_opencv.shape[0], axis=0)], axis=1)
        ...
        c2ws_opencv = c2ws_opencv[valid_mask]
        ...
        with open(gt_poses, 'w') as f:
            for c2w in gt_poses_np:
                f.write(f"{c2w[0][0]} {c2w[0][1]} {c2w[0][2]} {c2w[0][3]} {c2w[1][0]} {c2w[1][1]} {c2w[1][2]} {c2w[1][3]} {c2w[2][0]} {c2w[2][1]} {c2w[2][2]} {c2w[2][3]}\n")
        assert len(gt_poses_np) == len(c2ws_opencv), f"gt_poses_np: {len(gt_poses_np)}, c2ws_opencv: {len(c2ws_opencv)}"

        if not aligned_poses.exists() or not gt_poses.exists():
            print(f"Results not found for {scene_folder.name}.")
            continue

        eval_result = eval_pose_acc(gt_poses, aligned_poses, save_dir=scene_folder,conf_filter=-1)
