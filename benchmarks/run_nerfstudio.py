...
def export_point_cloud_from_nerfstudio(config_path: Path, output_folder: Path) -> None:
    ...
    cmd = "ns-export pointcloud " + " ".join(
        [f"--{k} {v}" for k, v in nerfstudio_args.items()]
    )
    print(f"Running command: {cmd}")
    run_command(cmd)


def fit_nerf_with_nerfstudio(
    nerf_data_path: Path,
    ...
    exp_name: str = "nerf_for_eval",
    max_num_iterations: int = 30000,
    ns_data_extra_args: Optional[Dict] = None,
) -> Path:
    ...
    dataparser_args = {"downscale-factor": downscale_factor, **ns_data_extra_args}
    ...
    cmd = (
        ...
        + method
        + " "
        + " ".join([f"--{k} {v}" for k, v in nerfstudio_args.items()])
    )
    cmd += " nerfstudio-data " + " ".join(
        [f"--{k} {v}" for k, v in dataparser_args.items()]
    )
...
def export_pose_after_ba(nerf_output_dir: Path, output_json: Path):
    ...
    print(f"Running command: {cmd}")
    run_command(cmd)
    return os.path.join(nerf_output_dir, "transforms_train.json"), os.path.join(
        nerf_output_dir, "transforms_eval.json"
    )
