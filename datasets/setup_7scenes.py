        ...
        def link_frames(split_file, variant):
            ...
            if opt.poses == "pgt":
                pgt_file = os.path.join(
                    "..", external_pgt_folder, "7scenes", f"{ds}_{variant}.txt"
                )
            ...
            for seq in split:
                ...
                if opt.poses == "original":
                    ...
                    for p_file in pose_files:
                        cam_pose, _ = pgt_poses[
                            os.path.join(seq, dutil.get_base_file_name(p_file))
                        ]
                        dutil.write_cam_pose(
                            target_folder + variant + "/poses/" + seq + "-" + p_file,
                            cam_pose,
                        )
                ...
                for d_file in depth_files:
                    if opt.poses == "pgt":
                        ...
                        _, rgb_f = pgt_poses[
                            os.path.join(seq, dutil.get_base_file_name(d_file))
                        ]
                    else:
                    ...
                    eye_tensor[0] = (
                        np.dstack([np.arange(0, out_w)] * out_h)[0].T * nn_subsampling
                        + nn_offset
                    )
                    eye_tensor[1] = (
                    ...
                    eye_tensor[
                        2, 0 : registered_depth.shape[0], 0 : registered_depth.shape[1]
                    ] = registered_depth
