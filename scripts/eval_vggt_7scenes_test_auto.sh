...
benchmarking_out_dir="benchmark_output/vggt_ba_1e-31e-5_2/7scenes_test_30k_50_150_300"
out_dir="reconstructions/7scenes_test_30k_50_150_300"
...
for scene in ${scenes[*]}; do
    input_rgb_files="${datasets_folder}/${scene}_test/seq-*/*.color.png"
    ...
    python benchmarks/convert_vggt_to_ace.py --pred_poses $vggt_pose_file   --data_pattern "${input_rgb_files}" --output_file $output_ace_file --intrinsics $intrinsics_file --split_json ${benchmarking_split_folder}/7scenes_${scene}_test.json --split_set test

    if $run_benchmark; then
        benchmarking_scene_dir="${benchmarking_out_dir}/${scene}"
        mkdir -p ${benchmarking_scene_dir}
        python -m benchmarks.benchmark_poses --pose_file ${output_ace_file} --output_dir ${benchmarking_scene_dir} --images_glob_pattern "${input_rgb_files}" --split_json ${benchmarking_split_folder}/7scenes_${scene}_test.json --method ${benchmarking_method} --camera_optimizer off --run_ba  2>&1 | tee ${benchmarking_out_dir}/log_${scene}.txt
    fi
done
...
# evo_ape kitti /mnt/jfs/hengli/projects/vggt_workspace/acezero/reconstructions/7scenes_test_30k_50_150_300/pumpkin/gt.txt /mnt/jfs/hengli/projects/vggt_workspace/acezero/reconstructions/7scenes_test_30k_50_150_300/pumpkin/pred.txt  -vas --save_plot /mnt/jfs/hengli/projects/vggt_workspace/acezero/reconstructions/7scenes_test_30k_50_150_300/pumpkin/pred
