...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base_seed", type=int, default=2089, help="seed to control randomness"
    )
    ...
    parser.add_argument(
        "--load_weights",
        type=Path,
        help="path to initialised network weights",
        default=None,
    )
    ...
    parser.add_argument(
        "--learning_rate_warmup_learning_rate",
        type=float,
        default=0.0005,
    ...
    if (
        options.use_pose_seed < 0
        and options.use_ace_pose_file is None
        and options.pose_files is None
    ):
        raise ValueError(
            "Either use_pose_seed or use_ace_pose_file or pose_files has to be set."
        )

    if (
        not options.use_heuristic_focal_length
        and options.use_external_focal_length is None
        and options.use_ace_pose_file is None
    ):
        raise ValueError(
            "Either use_heuristic_focal_length or use_external_focal_length "
